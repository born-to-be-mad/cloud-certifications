...
`Clustering` is a machine learning type that analyzes unlabeled data to find similarities in the data. +
`Regression` is a machine learning scenario that is used to predict numeric values. +
Data transformation is not a machine learning type.
...
Regression is used to predict numeric values. Clustering analyzes unlabeled data to find similarities in the data.
...
`The Clean Missing Data` module is part of preparing the data and data transformation process. +
`Select Columns in Dataset` is a data transformation component
that is used to choose a subset of columns of interest from a dataset. +
`Evaluate model` is a component used to measure the accuracy of trained models.